export const dishPolish = [
  {image:require('../assets/gamePlay/dishes/bigos.png'),
    id: 1,
    name: "Bigos (Hunter's Stew)",
    history: "Bigos is one of the most iconic dishes in Polish cuisine, with roots dating back to medieval times. Originally, it was a hunter's stew made with game meat, cabbage, and various spices. Over time, pork or beef replaced game meat, and the recipe evolved. Bigos was a favorite among the Polish nobility, especially during hunting trips. The dish is known for improving in flavor over several days as the ingredients meld together.",
    recipe: {
      ingredients: [
        "Sauerkraut - 1 kg",
        "Fresh cabbage - 0.5 kg",
        "Pork (shoulder or loin) - 500 g",
        "Polish sausage (smoked) - 300 g",
        "Dried mushrooms - 100 g",
        "Onion - 1",
        "Tomato paste - 3 tbsp",
        "Bay leaves - 2",
        "Black peppercorns - 10",
        "Juniper berries - 5",
        "Salt, sugar to taste",
        "Red wine - 200 ml (optional)"
      ],
      preparation: [
        "Dice the pork and sauté until golden.",
        "Add sliced sausage and cook for a few more minutes.",
        "Soak dried mushrooms in warm water for 30 minutes, then slice them thinly.",
        "In a large pot, combine both cabbages, the cooked meat, mushrooms, and spices. Cover with water or broth and simmer for 1-2 hours.",
        "Add tomato paste and red wine 20 minutes before the stew is done.",
        "Bigos is best served the next day when the flavors have had time to blend."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/dumplings.png'),
    id: 2,
    name: "Pierogi Ruskie (Dumplings with Potatoes and Cheese)",
    history: "Pierogi are a staple of Polish cuisine, and the \"Ruskie\" variety (meaning \"Ruthenian\") comes from the region of Rus, now part of modern-day Poland, Ukraine, and Belarus. These pierogi are traditionally filled with mashed potatoes and farmer's cheese, a simple and affordable combination. The dish became a favorite in rural areas and is often enjoyed at family gatherings and celebrations.",
    recipe: {
      ingredients: [
        "Flour - 500 g",
        "Egg - 1",
        "Water - 250 ml",
        "Potatoes - 500 g",
        "Farmer's cheese - 300 g",
        "Onion - 1",
        "Salt, pepper to taste",
        "Oil for frying"
      ],
      preparation: [
        "Boil and mash the potatoes, then allow to cool.",
        "Fry chopped onion in oil until golden.",
        "Mix the mashed potatoes with cheese and fried onions, seasoning with salt and pepper.",
        "Make the dough by combining flour, egg, and water, kneading until elastic.",
        "Roll out the dough and cut into circles.",
        "Place a bit of filling on each circle, fold, and seal the edges.",
        "Boil pierogi in salted water until they float to the top (about 3-5 minutes).",
        "Serve with fried onions or sour cream."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/zurek.png'),
    id: 3,
    name: "Żurek (Sour Rye Soup)",
    history: "Żurek is a traditional Polish soup made from fermented rye flour. Its history dates back to the rural traditions of medieval Poland, where fermented foods played a key role in the winter diet. Żurek is often served during Easter, symbolizing rebirth, and is usually paired with boiled eggs and sausage.",
    recipe: {
      ingredients: [
        "Rye sourdough starter - 500 ml",
        "Potatoes - 300 g",
        "White sausage - 300 g",
        "Eggs - 4",
        "Garlic - 3 cloves",
        "Bay leaves - 2",
        "Black peppercorns - 6",
        "Marjoram - 1 tsp",
        "Salt to taste"
      ],
      preparation: [
        "Make the rye starter a few days ahead by mixing rye flour with water and leaving it at room temperature to ferment.",
        "Boil potatoes separately.",
        "In a large pot, make broth with bay leaves and pepper.",
        "Add boiled potatoes and sausage to the broth.",
        "Pour in the rye starter and simmer until the soup thickens.",
        "Add marjoram and minced garlic, and cook for a few more minutes.",
        "Serve with hard-boiled eggs and sausage slices."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/kotlety.png'),
    id: 4,
    name: "Kotlety Schabowy (Breaded Pork Cutlets)",
    history: "Kotlety Schabowe is the Polish version of pork schnitzel, introduced in the 19th century under the influence of Western European cuisine. This dish quickly became popular among the Polish nobility, which is why they are sometimes called \"noble cutlets.\" Today, it is one of the most common meat dishes served in Poland, especially during family meals.",
    recipe: {
      ingredients: [
        "Pork chops - 4",
        "Eggs - 2",
        "Flour - 100 g",
        "Bread crumbs - 100 g",
        "Salt, pepper",
        "Oil or lard for frying"
      ],
      preparation: [
        "Pound the pork chops with a meat mallet, season with salt and pepper.",
        "Coat each chop in flour, dip in beaten eggs, and cover in bread crumbs.",
        "Fry the cutlets in hot oil or lard until golden brown on both sides (about 4-5 minutes per side).",
        "Serve with mashed potatoes and pickles or a side salad."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/peaSoup.png'),
    id: 5,
    name: "Kapusniak (Cabbage and Pea Soup)",
    history: "Kapusniak is a traditional cabbage soup that was widely prepared in peasant households. In winter, when fresh vegetables were scarce, fermented cabbage (sauerkraut) became a vital ingredient. Adding peas made the dish more filling and nutritious. Kapusniak was often served during the Lenten season as a meatless meal.",
    recipe: {
      ingredients: [
        "Sauerkraut - 500 g",
        "Dried peas - 200 g",
        "Onion - 1",
        "Carrot - 1",
        "Oil or lard - 2 tbsp",
        "Bay leaves - 2",
        "Salt, pepper to taste"
      ],
      preparation: [
        "Soak peas for several hours, then cook until soft.",
        "Sauté chopped onion and carrot in oil.",
        "Combine cooked peas with sauerkraut, onion, and carrots in a pot.",
        "Add bay leaves, salt, and pepper, and simmer for 20-30 minutes.",
        "Serve hot as a main dish or side."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/donuts.png'),
    id: 6,
    name: "Donuts (Polish Donuts)",
    history: "Donuts are a staple of Polish cuisine, especially popular on \"Fat Thursday,\" the last Thursday before Lent. This tradition, dating back several centuries, allowed people to indulge before the fasting period began. Originally, donuts were filled with savory ingredients, but over time, they became sweet, with fillings like jam or custard.",
    recipe: {
      ingredients: [
        "Flour - 500 g",
        "Eggs - 4",
        "Milk - 250 ml",
        "Yeast - 30 g",
        "Sugar - 100 g",
        "Butter - 100 g",
        "Jam or custard for filling",
        "Powdered sugar for dusting"
      ],
      preparation: [
        "Dissolve yeast in warm milk with a tablespoon of sugar, then let sit for 10-15 minutes.",
        "Beat eggs with sugar and add to the yeast mixture.",
        "Gradually add flour and melted butter, kneading the dough and leaving it to rise for 1 hour.",
        "Form dough balls, place a little jam or custard inside, and seal the edges.",
        "Fry the donuts in hot oil until golden brown.",
        "Dust with powdered sugar before serving."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/rolls.png'),
    id: 7,
    name: "Gołąbki (Stuffed Cabbage Rolls)",
    history: "Gołąbki, or stuffed cabbage rolls, are a versatile dish popular across Poland. Traditionally filled with minced meat and rice, they have been a staple of Polish family gatherings and holiday feasts for centuries. The name \"gołąbki\" means \"little pigeons,\" possibly referring to the rolled shape.",
    recipe: {
      ingredients: [
        "Cabbage leaves - 10",
        "Rice - 200 g",
        "Ground meat (pork or beef) - 300 g",
        "Onion - 1",
        "Tomato paste - 3 tbsp",
        "Sour cream - 200 ml",
        "Salt, pepper"
      ],
      preparation: [
        "Cook rice until half-done.",
        "Sauté chopped onion and mix with rice and ground meat, seasoning with salt and pepper.",
        "Blanch cabbage leaves in boiling water until soft.",
        "Place a spoonful of filling in each leaf, fold, and roll tightly.",
        "Place gołąbki in a pot, cover with diluted tomato paste, and simmer for 1-1.5 hours.",
        "Serve with sour cream."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/tripeSoup.png'),
    id: 8,
    name: "Flaki (Tripe Soup)",
    history: "Flaki is a traditional Polish soup made from beef tripe, dating back to the reign of King Władysław II Jagiełło in the 14th century. The dish was commonly served at grand feasts, and its nourishing qualities made it popular among soldiers. Despite its strong flavor, flaki remains a beloved dish in various regions of Poland.",
    recipe: {
      ingredients: [
        "Beef tripe - 1 kg",
        "Carrots - 2",
        "Onion - 1",
        "Celery root - 100 g",
        "Bay leaves - 2",
        "Black peppercorns - 10",
        "Marjoram - 1 tsp",
        "Salt"
      ],
      preparation: [
        "Rinse the tripe and simmer it in water for several hours until tender.",
        "Prepare broth with celery root, carrots, onion, bay leaves, and pepper.",
        "Cut the boiled tripe into thin strips and add to the broth.",
        "Simmer on low heat for another 1-2 hours, seasoning with salt and marjoram.",
        "Serve hot with bread."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/buckwheat.png'),
    id: 9,
    name: "Kaszotto (Buckwheat Dish)",
    history: "Buckwheat is an important ingredient in Polish culinary heritage, and dishes made with it have a long history. Kaszotto, a simple yet nourishing dish, is often found on Polish rural tables and is served as a side dish or main course with various additions like mushrooms or meat.",
    recipe: {
      ingredients: [
        "Buckwheat - 200 g",
        "Mushrooms - 200 g",
        "Onion - 1",
        "Sour cream - 3 tbsp",
        "Butter - 2 tbsp",
        "Salt, pepper"
      ],
      preparation: [
        "Cook buckwheat until tender.",
        "Sauté mushrooms and chopped onion in butter until golden.",
        "Add sour cream, salt, and pepper, cooking for a few more minutes.",
        "Mix the cooked buckwheat with the mushroom sauce.",
        "Serve hot as a side dish or main course."
      ]
    }
  },
  {image:require('../assets/gamePlay/dishes/applePie.png'),
    id: 10,
    name: "Szarlotka (Apple Pie)",
    history: "Szarlotka is a popular Polish apple pie that resembles the French dessert. The dish arrived in Poland in the 19th century, influenced by European cuisine. However, Poles adapted the recipe to their traditions, using abundant local apples. Szarlotka has become a classic dessert on Polish tables, especially in autumn when apples are in season.",
    recipe: {
      ingredients: [
        "Apples - 1 kg",
        "Flour - 400 g",
        "Butter - 200 g",
        "Sugar - 150 g",
        "Eggs - 2",
        "Cinnamon - 1 tsp",
        "Baking powder - 1 tsp"
      ],
      preparation: [
        "Cream butter with sugar, then add eggs and mix well.",
        "Combine flour with baking powder and incorporate into the butter mixture.",
        "Slice apples and sprinkle with cinnamon and a bit of sugar.",
        "Spread half the dough in a baking pan, layer with apples, and top with the remaining dough.",
        "Bake in a preheated oven at 180°C (356°F) for 40-45 minutes until golden.",
        "Serve cool, dusted with powdered sugar."
      ]
    }
  }
]
